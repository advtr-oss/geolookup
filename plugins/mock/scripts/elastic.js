...
module.exports = async (filePath, parser, opts = defaultOptions) => {
  ...
  const assign = b.expressionStatement(
    b.assignmentExpression("=", object.member('elastic', '$_connection'), functions.method('mock', 'getConnection'))
  )

  ast.program.body[1].expression.right.body.body.splice(0, 0, assign)
  ast.program.body[1].expression.right.params.splice(0, 0, identifier('mock'))
