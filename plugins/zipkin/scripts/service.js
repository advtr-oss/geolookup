...
module.exports = async (filePath, parser, options = defaultOptions) => {
  ...
  const requiredPath = b.literal('./utils/zipkin')
  const callee = b.identifier('require')
  const requireCallee = b.callExpression(callee, [requiredPath])
  const call = b.expressionStatement(b.callExpression(requireCallee, [b.identifier('config')]))
